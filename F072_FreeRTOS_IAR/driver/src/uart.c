 ...
 ** \brief  UART通道多主机模式配置
 **
 ** \param [in] UARTx通道号，stcMultiConfig多主机模式结构
 **
 ** \retval OK配置成功
 **\retval ErrorInvalidParameter配置失败
 ******************************************************************************/
en_result_t Uart_SetMultiMode(M0P_UART_TypeDef* UARTx, stc_uart_multimode_t* pstcMultiConfig)
{
    if(NULL != pstcMultiConfig)
    {
        UARTx->SCON_f.ADRDET = TRUE;   
        UARTx->SADDR = pstcMultiConfig->u8SlaveAddr;
        UARTx->SADEN = pstcMultiConfig->u8SaddEn;
 ...
 ** \brief  UART通道初始化函数
 **
 ** \param [in] UARTx通道号，pstcConfig初始化结构体 @ref stc_uart_config_t
 **
 ** \retval OK配置成功
 ** \retval ErrorInvalidParameter配置失败
 
 ******************************************************************************/
en_result_t Uart_Init(M0P_UART_TypeDef* UARTx, stc_uart_config_t* pstcConfig)
{
    ...
    UARTx->SCON = (uint32_t)pstcConfig->enStopBit |
                  (uint32_t)pstcConfig->enMmdorCk |
                  (uint32_t)pstcConfig->stcBaud.enClkDiv |
                  (uint32_t)pstcConfig->enRunMode;
    
    if((UartMskMode1 == pstcConfig->enRunMode) || (UartMskMode3 == pstcConfig->enRunMode))
    {
        u16OverShift = u32Over[pstcConfig->stcBaud.enClkDiv/UartMsk8Or16Div];
        UARTx->SCNT = pstcConfig->stcBaud.u32Pclk/(pstcConfig->stcBaud.u32Baud<<u16OverShift);
