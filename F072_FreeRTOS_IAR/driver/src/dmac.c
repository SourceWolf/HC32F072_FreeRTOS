...
**
** \param  [in] enCh                   指定通道.
** \param  [in] pstcConfig             DMAC通道初始化配置结构体指针.
**
** \retval Ok                          初始化成功.
** \retval ErrorInvalidParameter       pstcConfig是空指针.
**
** \note   None
**
******************************************************************************/
en_result_t Dma_InitChannel(en_dma_channel_t enCh, stc_dma_config_t* pstcConfig)
{
	...
	ASSERT(IS_VALID_SRC_ADDR_MODE(pstcConfig->enSrcAddrMode));
	ASSERT(IS_VALID_DST_ADDR_MODE(pstcConfig->enDstAddrMode));
	ASSERT(IS_VALID_PRIO_MODE(pstcConfig->enPriority));
	ASSERT(IS_VALID_TRANSFER_MODE(pstcConfig->enTransferMode));
	  
	/* 检查通道值有效性和pstcConfig是否空指针 */
	if (NULL == pstcConfig)
	{
  ...
  *(&M0P_DMAC->CONFB0+enCh) = 0;
	*(&M0P_DMAC->CONFB0+enCh) = (uint32_t)pstcConfig->enMode             |
		                          (uint32_t)pstcConfig->enTransferWidth    |
		                          (uint32_t)pstcConfig->enSrcAddrMode      |
		                          (uint32_t)pstcConfig->enDstAddrMode      |
		                          (uint32_t)pstcConfig->enSrcAddrReloadCtl |
		                          (uint32_t)pstcConfig->enDestAddrReloadCtl|
		                          (uint32_t)pstcConfig->enSrcBcTcReloadCtl |
		                          (uint32_t)pstcConfig->enTransferMode;

	/*首先把TRI_SEL[6:0]     BC[3:0] TC[15:0]这些位清零，然后再赋值*/
	*(&M0P_DMAC->CONFA0+enCh) &= ((uint32_t)~(DMA_TRI_SEL_Msk | DMA_BC_SEL_Msk | DMA_TC_SEL_Msk));
	*(&M0P_DMAC->CONFA0+enCh)	|= (uint32_t)(pstcConfig->u16TransferCnt - 1)    |
		                           ((uint32_t)(pstcConfig->u16BlockSize - 1)<<16)|
		                           (uint32_t)(pstcConfig->enRequestNum<<22);		

	M0P_DMAC->CONF |= (uint32_t)(pstcConfig->enPriority);						   
	
  *(&M0P_DMAC->SRCADR0+enCh) = (uint32_t)(pstcConfig->u32SrcAddress);
	*(&M0P_DMAC->DSTADR0+enCh) = (uint32_t)(pstcConfig->u32DstAddress);
...
{
  *(&M0P_DMAC->CONFA0+enCh) = (*(&M0P_DMAC->CONFA0+enCh)&((uint32_t)~DMA_BC_SEL_Msk))|((((uint32_t)u16BlkSize-1)&0x0f)<<DMA_BC_SEL_Pos);
}
...
{
  *(&M0P_DMAC->CONFA0+enCh) = (*(&M0P_DMAC->CONFA0+enCh)&((uint32_t)~DMA_TC_SEL_Msk))|(((uint32_t)(u16TrnCnt-1)<<DMA_TC_SEL_Pos));
}
...
{
  *(&M0P_DMAC->CONFB0+enCh) = (*(&M0P_DMAC->CONFB0+enCh)&((uint32_t)~DMA_TRANSFER_WIDTH_Msk))|((uint32_t)enWidth);
}
/**
*******************************************************************************
** \brief  设定dma通道优先级.
