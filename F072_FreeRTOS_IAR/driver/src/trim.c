...
en_result_t Trim_Init(stc_trim_config_t* pstcConfig)
{
    ...
    M0P_CLK_TRIM->CR = (uint32_t)pstcConfig->enCALCLK |
                       (uint32_t)pstcConfig->enREFCLK |
                       (uint32_t)pstcConfig->enMON;  
    
    M0P_CLK_TRIM->REFCON = pstcConfig->u32RefCon;
    M0P_CLK_TRIM->CALCON = pstcConfig->u32CalCon;
