 ...
 **
 ** \param [in] LPUARTx通道号，stcMultiConfig多主机模式结构
 **
 ** \retval OK配置成功
 **\retval ErrorInvalidParameter配置失败
 ******************************************************************************/
en_result_t LPUart_SetMultiMode(M0P_LPUART_TypeDef* LPUARTx, stc_lpuart_multimode_t* pstcMultiConfig)
{
     
    if(NULL != pstcMultiConfig)
    {
        LPUARTx->SCON_f.ADRDET = TRUE;
        LPUARTx->SADDR = pstcMultiConfig->u8SlaveAddr;
        LPUARTx->SADEN = pstcMultiConfig->u8SaddEn;
    }
    else
    {
 ...
 ** \brief  LPUART通道初始化函数
 **
 ** \param [in] u8Idx通道号，pstcConfig初始化结构体  @ref stc_lpuart_config_t
 **
 ** \retval OK配置成功
 **\retval ErrorInvalidParameter配置失败
 ******************************************************************************/
en_result_t LPUart_Init(M0P_LPUART_TypeDef* LPUARTx,stc_lpuart_config_t* pstcConfig)
{
    ...
    LPUARTx->SCON = (uint32_t)pstcConfig->enStopBit          |
                    (uint32_t)pstcConfig->enMmdorCk          |
                    (uint32_t)pstcConfig->stcBaud.enSclk_Div |
                    (uint32_t)pstcConfig->stcBaud.enSclkSel  |
                    (uint32_t)pstcConfig->enRunMode;
    
    if((LPUartMskMode1 == pstcConfig->enRunMode) || (LPUartMskMode3 == pstcConfig->enRunMode))
    {
        u16OverShift = u32Over[pstcConfig->stcBaud.enSclk_Div/LPUartMsk8Or16Div];
        LPUARTx->SCNT = pstcConfig->stcBaud.u32Sclk/(pstcConfig->stcBaud.u32Baud<<u16OverShift);
