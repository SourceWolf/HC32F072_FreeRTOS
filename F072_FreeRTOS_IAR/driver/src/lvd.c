...
void Lvd_Init(stc_lvd_config_t *pstcConfig)
{
    ...
    M0P_LVD->CR = (uint32_t)pstcConfig->enAct        |
                  (uint32_t)pstcConfig->enFilter     |
                  (uint32_t)pstcConfig->enFilterTime |
                  (uint32_t)pstcConfig->enInputSrc   |
                  (uint32_t)pstcConfig->enIrqType    |
                  (uint32_t)pstcConfig->enThreshold;
}
