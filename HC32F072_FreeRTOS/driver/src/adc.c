...
en_result_t Adc_Init(stc_adc_cfg_t* pstcAdcConfig)
{
    ...
    M0P_ADC->CR0 |= (uint32_t)pstcAdcConfig->enAdcClkDiv       |
                    (uint32_t)pstcAdcConfig->enAdcRefVolSel    |
                    (uint32_t)pstcAdcConfig->enAdcOpBuf        |
                    (uint32_t)pstcAdcConfig->enAdcSampCycleSel |
                    (uint32_t)pstcAdcConfig->enInRef;

    M0P_ADC->CR1_f.MODE  = pstcAdcConfig->enAdcMode;
    M0P_ADC->CR1_f.ALIGN = pstcAdcConfig->enAdcAlign;
...
void Adc_SglExtTrigCfg(en_adc_trig_sel_t enAdcTrigSel, boolean_t bValue)
{
    if(TRUE == bValue)
    {
        M0P_ADC->EXTTRIGGER0 |= enAdcTrigSel;
    }
    else
    {
...
void Adc_JqrExtTrigCfg(en_adc_trig_sel_t enAdcTrigSel, boolean_t bValue)
{
    if(TRUE == bValue)
    {
        M0P_ADC->EXTTRIGGER1 |= enAdcTrigSel;
    }
    else
    {
 ...
 * \retval  en_result_t  ErrorInvalidParameter: 无效参数
 */
en_result_t Adc_SqrModeConfig(stc_adc_sqr_cfg_t* pstcAdcSqrConfig)
{
    if ((NULL == pstcAdcSqrConfig) || (pstcAdcSqrConfig->u8SqrCnt > 16))
    {
        return ErrorInvalidParameter;
    }

    M0P_ADC->CR1_f.RACCCLR = 0; //ADC转换结果累加寄存器（ADC_ResultAcc）清零
    M0P_ADC->CR1_f.RACCEN  = pstcAdcSqrConfig->enResultAcc;
    M0P_ADC->CR1_f.DMASQR  = pstcAdcSqrConfig->bSqrDmaTrig;
    
    M0P_ADC->SQR2_f.CNT  = pstcAdcSqrConfig->u8SqrCnt - 1;
 ...
 * \retval  en_result_t  ErrorInvalidParameter: 无效参数
 */
en_result_t Adc_JqrModeConfig(stc_adc_jqr_cfg_t* pstcAdcJqrConfig)
{
    if ((NULL == pstcAdcJqrConfig) || (pstcAdcJqrConfig->u8JqrCnt > 4))
    {
        return ErrorInvalidParameter;
    }

    M0P_ADC->CR1_f.DMASQR  = pstcAdcJqrConfig->bJqrDmaTrig;
    
    M0P_ADC->JQR_f.CNT  = pstcAdcJqrConfig->u8JqrCnt - 1;
...
void Adc_ThresholdConfig(stc_adc_threshold_cfg_t* pstcAdcThrCfg)
{
