 ...
 **
 ** \param [in] SPIx 通道
 ** \param [in] pstcSpiConfig 初始化结构
 **
 ** \retval Ok初始化成功
 ** \retval ErrorInvalidParameter 初始化错误
 ******************************************************************************/
en_result_t Spi_Init(M0P_SPI_TypeDef* SPIx, stc_spi_config_t *pstcSpiConfig)
{
    ASSERT(NULL != pstcSpiConfig);

    SPIx->CR = 0;
    SPIx->CR =  (uint32_t)pstcSpiConfig->enSpiMode |
                (uint32_t)pstcSpiConfig->enPclkDiv |
                (uint32_t)pstcSpiConfig->enCPOL    |
                (uint32_t)pstcSpiConfig->enCPHA    |
                (uint32_t)0x40;
